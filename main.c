...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance,
    ...
    // 单实例检测：使用命名互斥体检查是否已有实例在运行
    HANDLE hMutex = CreateMutexW(NULL, TRUE, L"SharedVoice_SingleInstance_Mutex");
    if (hMutex == NULL || GetLastError() == ERROR_ALREADY_EXISTS) {
        MessageBoxW(NULL, L"程序已经在运行中！", L"SharedVoice", MB_OK | MB_ICONINFORMATION);
